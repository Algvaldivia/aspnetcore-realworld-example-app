            ...
            steps {
                withSonarQubeEnv(installationName: 'SonarQube (172.1.203.230)', envOnly: true) {
                    ...
                    sh label: 'Running dotnet-sonarscanner', script: "\
                        ...
                        && if [[ ! -f \"/root/.dotnet/tools/dotnet-sonarscanner\" ]]; then dotnet tool install dotnet-sonarscanner -g; fi \
                        && /root/.dotnet/tools/dotnet-sonarscanner begin /k:${JOB_BASE_NAME} /name:${JOB_NAME} /version:${BUILD_NUMBER} /d:sonar.login=${SONAR_AUTH_TOKEN} /d:sonar.verbose=true /d:sonar.host.url=${SONAR_HOST_URL} \
                        && dotnet build \"${WORKSPACE}\" \
                        && /root/.dotnet/tools/dotnet-sonarscanner end /d:sonar.login=${SONAR_AUTH_TOKEN} \
                    "
